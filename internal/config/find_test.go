...
func TestFindConfigFile(t *testing.T) {
	t.Run(("finds config in current directory"), func(t *testing.T) {
		...
		got, err := config.FindConfigFile(fs, "/project")
	...
	t.Run("finds config when starting from file path", func(t *testing.T) {
		...
		require.NoError(t, afero.WriteFile(fs, "/project/nested/.seil.yml", []byte("setup: {}\n"), 0o600))
		...
		require.NoError(t, err)
		assert.Equal(t, "/project/nested/.seil.yml", got)
	})
